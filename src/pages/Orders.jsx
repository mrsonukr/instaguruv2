...
const Orders = () => {
  ...
  const getTimeAgo = (createdAt) => {
    ...
    if (minutesDiff < 60) return `${minutesDiff} ${getTranslation('minAgo', language)}`;
    
    const hoursDiff = Math.floor(minutesDiff / 60);
    if (hoursDiff < 24) return `${hoursDiff} ${hoursDiff > 1 ? getTranslation('hoursAgo', language) : getTranslation('hourAgo', language)}`;
    
    const daysDiff = Math.floor(hoursDiff / 24);
    return `${daysDiff} ${daysDiff > 1 ? getTranslation('daysAgo', language) : getTranslation('dayAgo', language)}`;
  };
    ...
    if (serviceLower === 'instagram') {
      return (
        <div className="w-8 h-8 rounded-full flex items-center justify-center">
          <img src="/ic/insta.webp" alt="Instagram" className="w-8 h-8 rounded-full" />
        </div>
      );
    } else if (serviceLower === 'youtube') {
      return (
        <div className="w-8 h-8 rounded-full flex items-center justify-center">
          <img src="/ic/youtube.webp" alt="YouTube" className="w-8 h-8 rounded-full" />
        </div>
      );
    } else if (serviceLower === 'facebook') {
      return (
        <div className="w-8 h-8 rounded-full flex items-center justify-center">
          <img src="/ic/facebook.webp" alt="Facebook" className="w-8 h-8 rounded-full" />
        </div>
      );
    } else if (serviceLower === 'netflix') {
      return (
        <div className="w-8 h-8 rounded-full flex items-center justify-center">
          <img src="/ic/netflix.svg" alt="Netflix" className="w-8 h-8 rounded-full" />
        </div>
      );
    } else if (serviceLower === 'spotify') {
      return (
        <div className="w-8 h-8 rounded-full flex items-center justify-center">
          <img src="/ic/spotify.webp" alt="Spotify" className="w-8 h-8 rounded-full" />
        </div>
      );
    } else if (serviceLower === 'amazon prime' || serviceLower === 'amazon-prime') {
      return (
        <div className="w-8 h-8 rounded-full flex items-center justify-center">
          <img src="/ic/prime.webp" alt="Amazon Prime" className="w-8 h-8 rounded-full" />
        </div>
      );
    } else if (serviceLower === 'telegram') {
      return (
        <div className="w-8 h-8 rounded-full flex items-center justify-center">
          <img src="/ic/telegram.webp" alt="Telegram" className="w-8 h-8 rounded-full" />
        </div>
      );
    } else {
      // Fallback to partial matches
      if (serviceLower.includes('instagram') || serviceLower.includes('insta')) {
        return (
          <div className="w-8 h-8 rounded-full flex items-center justify-center">
            <img src="/ic/insta.webp" alt="Instagram" className="w-8 h-8 rounded-full" />
          </div>
        );
      } else if (serviceLower.includes('youtube') || serviceLower.includes('yt')) {
        return (
          <div className="w-8 h-8 rounded-full flex items-center justify-center">
            <img src="/ic/youtube.webp" alt="YouTube" className="w-8 h-8 rounded-full" />
          </div>
        );
      } else if (serviceLower.includes('facebook') || serviceLower.includes('fb')) {
        return (
          <div className="w-8 h-8 rounded-full flex items-center justify-center">
            <img src="/ic/facebook.webp" alt="Facebook" className="w-8 h-8 rounded-full" />
          </div>
      ...
      } else if (serviceLower.includes('amazon') || serviceLower.includes('prime')) {
        return (
          <div className="w-8 h-8 rounded-full flex items-center justify-center">
            <img src="/ic/prime.webp" alt="Amazon Prime" className="w-8 h-8 rounded-full" />
          </div>
        );
      } else if (serviceLower.includes('telegram') || serviceLower.includes('tg')) {
        return (
          <div className="w-8 h-8 rounded-full flex items-center justify-center">
            <img src="/ic/telegram.webp" alt="Telegram" className="w-8 h-8 rounded-full" />
          </div>
